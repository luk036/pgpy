"""Release data for PgPy.

When PgPy is imported a number of steps are followed to determine
the version information.
   ...
   1) If the release is not a development release (dev=False), then version
      ...
      downloadable release of PgPy since setup.py creates it during
      packaging/installation.  However, version.py might not exist if one
      is running PgPy from the mercurial repository.  In the event that
...
PgPy, we must make sure write_versionfile() does not overwrite the
...
def write_versionfile():
    ...
    text = '''"""
Version information for PgPy, created during installation.
...
# Was PgPy built from a development version? If so, remember that the major
...
def get_info(dynamic=True):
    ...
    if dynamic_failed or not dynamic:
        # This is where most final releases of PgPy will be.
...
long_description = \
    """
PgPy is a Python package for the creation, manipulation, and
...
url = 'http://pgpy.github.io/'
download_url = 'https://pypi.python.org/pypi/pgpy/'
